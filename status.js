...
var dangerousCommands = ["eval","pullanddeploy","setUsername","cmdauth","alias","aliases","ban","bans","unban","votekick","msg","say"];
var Permissions = {};
try{
...
commands = {
    ...
    deleteAfter: true,
        ...
        process: function(bot,msg,suffix){ msg.channel.send(suffix).then((message => msg.delete(50)));}
     },
...
if(AuthDetails.hasOwnProperty("client_id")){
	commands["invite"] = {
		...
		process: function(bot,msg,suffix){
			msg.channel.send("invite link: https://discordapp.com/oauth2/authorize?&client_id=" + AuthDetails.client_id + "&scope=bot&permissions=21469585838");
		}
	}
}
        ...
        if(cmdTxt === "help"){
			...
			if(Permissions.checkPermission(msg.author.id,cmdTxt)){
				try{
					...
					var msgTxt = "command " + cmdTxt + " failed. So sorry ";
					if(Config.debug){
						 msgTxt += "\n" + e.stack;
						 console.log(msgTxt);
					}
				...
				msg.channel.send("You are not allowed to run " + cmdTxt + "! Senpai forbids it!");
			}
			return true;
		} else {
